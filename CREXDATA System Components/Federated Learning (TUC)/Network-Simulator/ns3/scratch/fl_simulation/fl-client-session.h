...
public:
  /**
        ...
        */
  ClientSession (int clientID_, double x_, double y_);
  ...
  int GetLocalSteps ();
  void SetLocalSteps (int _localSteps);
...
private:
  ...
  int m_localSteps; //!< Indicates whether client should participate in round
...
private:
  ...
  std::map<int, std::shared_ptr<ClientSession>>
      &m_clientSessionById; //!< maps client id to client session
