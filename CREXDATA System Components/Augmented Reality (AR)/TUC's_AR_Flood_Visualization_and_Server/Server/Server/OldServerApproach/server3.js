...
const CONFIG = {
  ...
  KAFKA_BROKERS: [process.env.KAFKA_BROKER || "server.crexdata.eu:9092"],
  KAFKA_TOPIC: process.env.KAFKA_TOPIC || "kpler-crex-maritime-ca-result", // Or your new topic
  KAFKA_USERNAME: process.env.KAFKA_USERNAME || "user",
  KAFKA_PASSWORD: process.env.KAFKA_PASSWORD || "password",
  KAFKA_AUTH_MECHANISM: "plain",
  CLIENT_ID: "TUC4",

  // Paths to your PEM files (after converting from JKS)
  // You'll need to replace these with the actual paths to your converted PEM files
  KAFKA_SSL_CA_PATH: process.env.KAFKA_SSL_CA_PATH || "./certs/ca.crt.pem",
  KAFKA_SSL_KEY_PATH: process.env.KAFKA_SSL_KEY_PATH || "./certs/client.key.pem",
  KAFKA_SSL_CERT_PATH: process.env.KAFKA_SSL_CERT_PATH || "./certs/client.crt.pem",
  // If your client.key.pem is passphrase protected
  KAFKA_SSL_KEY_PASSPHRASE: process.env.KAFKA_SSL_KEY_PASSPHRASE || null, // Set if your key has a passphrase
...
async function initializeKafkaConnection() {
  try {
    ...
    const sslOptions = {
      rejectUnauthorized: process.env.NODE_ENV === "production", // Be careful with this in production
      ca: [fs.readFileSync(CONFIG.KAFKA_SSL_CA_PATH, "utf-8")],
      key: fs.readFileSync(CONFIG.KAFKA_SSL_KEY_PATH, "utf-8"),
      cert: fs.readFileSync(CONFIG.KAFKA_SSL_CERT_PATH, "utf-8"),
    };
    if (CONFIG.KAFKA_SSL_KEY_PASSPHRASE) {
      sslOptions.passphrase = CONFIG.KAFKA_SSL_KEY_PASSPHRASE;
    }
    ...
    const kafka = new Kafka({
      ...
      ssl: sslOptions,
      sasl: {
        mechanism: CONFIG.KAFKA_AUTH_MECHANISM, // 'plain'
      ...
      ws.on("message", async (message) => {
        ...
        try {
          ...
          switch (data.type) {
            ...
            //sendToAll(messageStr); // Broadcast to all clients
            //handleMarineMessage(ws, messageStr, data, consumer);
              ...
              eachMessage: async ({ topic, partition, message }) => {
                ...
                if (ws.readyState === WebSocket.OPEN) {
                  try {
                    ...
                    if (kafkaData.hasOwnProperty("ship_mmsi")) logger.debug("Kafka message data", kafkaData);
...
handleMarineTrafficData = (ws, data, message) => {
  try {
    logger.debug("Processing marine traffic data", { data, message });

    // Send the ship data to the all MarineCLients client
    sendToAll(ws, data);
    logger.warn("Invalid marine traffic data format:", message);
  } catch (error) {
    logger.error("Error processing marine traffic data:", error);
  }
};
