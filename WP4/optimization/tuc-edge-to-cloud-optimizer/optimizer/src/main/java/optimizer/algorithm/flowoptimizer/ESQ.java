...
public class ESQ {
    ...
    public static Graph createPlanSpaceWithQueue(Graph flow, Map<Integer, AvailablePlatform> platformMapping, Map<Integer, Site> siteMapping,
        ...
        ESQ.costEstimation = costEstimation;
        //Get the sizes
            ...
            while (!planSpace_X.isEmpty()) {
                ...
                for (Vertex v : g.getVertices()) {
                    ...
                    for (List<Integer> e : actions) {
                        ...
                        OptimizerAction actionToConsider = allAction[e.get(2)];

                        // same-setup filter: skip, if the vertex already runs on P or S
                        switch (actionToConsider) {
                            case CHANGE_SITE:
                                if (v.getSite() == e.get(0)) continue;
                                break;
                            case CHANGE_PLATFORM:
                                if (v.getPlatform() == e.get(1)) continue;
                                break;
                            default:
