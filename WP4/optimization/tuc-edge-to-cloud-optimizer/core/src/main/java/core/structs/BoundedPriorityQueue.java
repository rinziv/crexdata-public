...
public class BoundedPriorityQueue<E> extends AbstractSet<E> implements Queue<E>, SortedSet<E> {
     ...
     * @param threadSafe  If true then all then this object is thread safe..
     * @throws IllegalArgumentException If the maximum size is less than 1.
     */
    public BoundedPriorityQueue(Comparator<? super E> comparator, int maxSize, boolean threadSafe) {
        if (maxSize < 1) {
            String msg = "Require maximum size >= 1."
                    + " Found max size=" + maxSize;
            throw new IllegalArgumentException(msg);
        }
        mQueue = threadSafe ? Collections.synchronizedSortedSet(new TreeSet<>(new EntryComparator())) : new TreeSet<>(new EntryComparator());
     ...
     *
     * @return True if the PQ is at max capacity, false otherwise.
     */
