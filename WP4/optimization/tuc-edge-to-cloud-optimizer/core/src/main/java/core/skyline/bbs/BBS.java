package core.skyline.bbs;

import core.structs.rtree.*;
...
public class BBS {
    ...
    private final RTree<Object, Point> rTree;
    ...
    private boolean isDominated(BBSHeapElement a, Entry<Object, Point> b) {
        return a.isNode()
                ? b.geometry().x() <= a.getNode().geometry().mbr().x1() && (b.geometry().y() <= a.getNode().geometry().mbr().y1())
                : (b.geometry().x() <= a.getEntry().geometry().mbr().x1()) && b.geometry().y() <= a.getEntry().geometry().mbr().y1();
    }
    ...
    private boolean isDominatedInSet(BBSHeapElement a, List<Entry<Object, Point>> entries) {
        return entries.stream().anyMatch(entry -> isDominated(a, entry));
    }
